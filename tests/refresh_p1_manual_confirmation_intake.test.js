...
const {
    ...
    main,
    resolveArgs
} = require("../scripts/refresh_p1_manual_confirmation_intake.js");
...
function buildManualConfirmationFixture() {
    return {
        ...
        fresh_capture_templates: [
            {
                source_task_id: "capture_full_count_sunken_ship_p1_case",
...
test("package exposes one-command P1 manual confirmation intake wrapper", () => {
    assert.equal(
        packageJson.scripts["intake:p1-manual-confirmation"],
        "node scripts/refresh_p1_manual_confirmation_intake.js"
    );
    assert.equal(FOCUS_LABEL, "P1");
    assert.equal(SCRIPT_NAME, "intake:p1-manual-confirmation");
    assert.match(packageJson.scripts["check:js"], /scripts\/refresh_p1_manual_confirmation_intake\.js/);
});

test("resolveArgs accepts source and isolated P1 output paths", () => {
...
test("default output path targets P1 manual confirmation intake refresh artifact", () => {
    assert.equal(
        DEFAULT_PATHS.outputPath.endsWith("2026-04-27-sunken-ship-p1-manual-confirmation-intake-refresh.json"),
        true
    );
});

test("main runs ingest and handoff gate into a compact P1 intake report", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ak-p1-intake-"));
    ...
    assert.equal(report.schema_version, "ak_p1_manual_confirmation_intake_refresh_v1");
    assert.equal(report.focus_label, "P1");
    ...
    assert.match(report.commands.rerun_intake, /intake:p1-manual-confirmation/);
    assert.match(report.commands.next_human_step, /P1 manual count confirmation HTML/);
    ...
    assert.match(markdown, /P1 Manual Confirmation Intake Refresh/);
